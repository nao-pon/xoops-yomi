<?php
// $Id: x20_keepblockoptions.inc.php,v 1.1 2009/03/05 04:24:46 nao-pon Exp $
...
if ($record) {
	...
	for ($i = 1 ; $i <= $count ; $i++) {
		$sql = "SELECT name,options FROM ".$db->prefix('newblocks')." WHERE mid=".$mid." AND func_num=".$i." AND show_func=".$xoopsDB->quoteString($modversion['blocks'][$i]['show_func'])." AND func_file=".$xoopsDB->quoteString($modversion['blocks'][$i]['file']);
		$fresult = $db->query($sql);
